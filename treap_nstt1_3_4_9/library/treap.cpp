...
template <typename T>
static void split_treap(typename Treap<T>::Node* root, const T& key, typename Treap<T>::Node** left, typename Treap<T>::Node** right) {
    if (root == nullptr) {
        *left = nullptr;
        *right = nullptr;
        return;
...
template <typename T>
static typename Treap<T>::Node* merge_nodes(typename Treap<T>::Node* t1, typename Treap<T>::Node* t2) {
    if (t1 == nullptr)
        return t2;
